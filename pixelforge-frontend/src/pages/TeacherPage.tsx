...
// Создание и публикация заданий переехали в студию методиста на портале
// (learning-portal, раздел /pixelforge). Эта страница теперь — рабочее
// место тренера: список заданий класса и проверка сдач.
...
export function TeacherPage() {
  ...
  useEffect(() => {
    fetchStaffClasses(HARDCODED_TEACHER_ID, 'TRAINER').then((list) => {
      setClasses(list);
  ...
  useEffect(() => {
    ...
    listAllAssignments(selectedClassId).then(setAssignments).catch(() => {});
  }, [selectedClassId]);
  ...
  return (
    <div className="page">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '16px' }}>
        <h1 style={{ marginBottom: 0 }}>Рабочее место тренера</h1>
        {classes.length > 1 && (
      ...
      <div className="card">
        <h2 style={{ fontSize: '16px' }}>Задания класса</h2>
